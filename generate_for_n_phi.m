...
for n_phi = 1:1:6
    ...
    for i=1:size(orients, 2)
        ...
        % Save right parts
        matr_name = strcat({'b_'}, orient, {'_n_phi_'}, num2str(n_phi), {'.txt'}); 
        ...
        % Save model solution
        sol_name = strcat({'x_'}, orient, {'_n_phi_'}, num2str(n_phi), {'.txt'}); 
        ...
        title(title_str)
        fpath = 'C:\Users\Daria\Documents\MATLAB\projections';
        ...
        title(title_str)
        fpath = 'C:\Users\Daria\Documents\MATLAB\variables';
        saveas(gcf, fullfile(fpath, char(strcat(title_str, '.png'))), 'png');
    end
    ...
    matr_name = strcat({'b'}, {'_n_phi_'}, num2str(n_phi), {'.txt'}); 
    matr_name = char(matr_name);
    save(matr_name, 'bsol216', '-ascii');
    
    % Save summary solution
    sol_name = strcat({'x'},{'_n_phi_'}, num2str(n_phi), {'.txt'}); 
    sol_name = char(sol_name);
    save(sol_name, 'test_sol216', '-ascii');
    ...
    fpath = 'C:\Users\Daria\Documents\MATLAB\summary';
    saveas(gcf, fullfile(fpath, char(strcat(title_str, '.png'))), 'png');
end
